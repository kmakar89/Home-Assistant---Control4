...
def setup(hass, config):
    ...
    def handle_garage_amp_off_select(call):
        ...
        send_udp_command("c4.amp.out 01 00", "192.168.5.11", 8750)
    ...
    def handle_garage_amp_on_select(call):
        ...
        send_udp_command("c4.amp.chvol 01 a9", "192.168.5.11", 8750)
    ...
    def handle_gr_amp_off_select(call):
        #_LOGGER.warn("Powering AMP Off...")
        send_udp_command("c4.amp.out 03 00", "192.168.5.11", 8750)
        send_udp_command("c4.amp.out 04 00", "192.168.5.11", 8750)
        #_LOGGER.warn("AMP Powered Off...")
    ...
    def handle_gr_amp_on_select(call):
        ...
        send_udp_command("c4.amp.chvol 03 a9", "192.168.5.11", 8750)
        send_udp_command("c4.amp.chvol 04 a9", "192.168.5.11", 8750)
        send_udp_command("c4.amp.out 03 0" + amp_input, "192.168.5.11", 8750)
        send_udp_command("c4.amp.out 04 0" + amp_input, "192.168.5.11", 8750)
        #_LOGGER.warn("AMP Powered On...")
    ...
    def handle_gr_tuner_channel_select(call):
        ...
        tuner_station = hex(int(tuner_station))[2:]
        # tafreq2 = Tuner 1 | tbfreq2 = Tuner 2
        send_udp_command("c4.mt.tbfreq2 00 " + str(tuner_station), "192.168.5.14", 8750)
        #_LOGGER.warn("Changed tuner channel...")
    ...
    def handle_gr_amp_volume_select(call):
        ...
        volume_select = int(float(volume_select) * 100) + 160
        volume_select = hex(volume_select)[2:]
        send_udp_command("c4.amp.chvol 03 " + volume_select, "192.168.5.11", 8750)
        send_udp_command("c4.amp.chvol 04 " + volume_select, "192.168.5.11", 8750)
        #_LOGGER.warn("Volume set...")
    ...
    def handle_lanai_amp_off_select(call):
        #_LOGGER.warn("Powering AMP Off...")
        send_udp_command("c4.amp.out 02 00", "192.168.5.11", 8750)
        #_LOGGER.warn("AMP Powered Off...")
    ...
    def handle_lanai_amp_on_select(call):
        ...
        tuner_station = hass.states.get("input_select.gr_radio_stations").state
        send_udp_command("c4.amp.chvol 02 a9", "192.168.5.11", 8750)
        send_udp_command("c4.amp.out 02 0" + amp_input, "192.168.5.11", 8750)
        #_LOGGER.warn("AMP Powered On...")
    ...
    def handle_lanai_amp_volume_select(call):
        ...
        volume_select = int(float(volume_select) * 100) + 160
        volume_select = hex(volume_select)[2:]
        send_udp_command("c4.amp.chvol 02 " + volume_select, "192.168.5.11", 8750)
        #_LOGGER.warn("Volume set...")
